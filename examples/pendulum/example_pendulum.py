...
def generate_pendulum_data(n_ics):
    f  = lambda z, t : [z[1], -np.sin(z[0])]  # [dtheta/dt, ddtheta/dt^2]
    t = np.arange(0, 10, .02)
    ...
    while (i < n_ics):
        ...
        if np.abs(z0[1]**2/2. - np.cos(z0[0])) > .99:
            continue
        z[i] = odeint(f, z0, t) # [theta, dtheta/dt]
        dz[i] = np.array([f(z[i,j], t[j]) for j in range(len(t))]) # [dtheta/dt, ddtheta/dt^2]
        i += 1
...
def pendulum_to_movie(z, dz):
    ...
    # second derivative of contents of exp
    argument_derivative2 = lambda theta, dtheta, ddtheta : -2/.05*((np.sin(theta-np.pi/2))*np.sin(theta-np.pi/2)*dtheta**2 \
